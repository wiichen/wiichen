...
def main(config_file):
    ...
    print(f"测试连接到交换机 {host}:{port}")
    
    # 连接设备
    m = huawei_connect(host, port, username, password)
    if m:
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("用法: python test_connection.py <配置文件路径>")
        print("示例: python test_connection.py switch_config.json")
        sys.exit(1)
