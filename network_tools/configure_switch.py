...
def main(config_file):
    ...
    print(f"连接到交换机 {host}:{port}")
    
    # 连接设备并配置
    with huawei_connect(host, port, username, password) as m:
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("用法: python configure_switch.py <配置文件路径>")
        print("示例: python configure_switch.py switch_config.json")
        sys.exit(1)
